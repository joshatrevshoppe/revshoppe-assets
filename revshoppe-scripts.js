document.addEventListener('DOMContentLoaded', function() {
    ...
    var menuToggling = false;
    function toggleMenu() {
      if (menuToggling) return;
      menuToggling = true;
      setTimeout(function() { menuToggling = false; }, 300);
...
document.addEventListener('DOMContentLoaded', function() {
      // Randomize which quote opens on load
...
(function() {
  ...
  // ═══════════════════════════════════════════════
  // 3. TRIPLE-CLICK BOLT LOGO → "CHARGED."
  // ═══════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════
  // 4. FULLSCREEN → GTM DIAGNOSIS OVERLAY
  // Triggers when user enters fullscreen (green button on Mac / F11)
  // ═══════════════════════════════════════════════
  ...
  window.triggerDiagnosis = function triggerDiagnosis() {
    ...
    var chars = '01';
    var frameCount = 0;
    var maxFrames = 95; // ~3.8s at 40ms intervals
    var rafId;
    ...
    function drawRain() {
      ...
      if (frameCount < maxFrames) {
        rafId = setTimeout(drawRain, 40);
      } else {
        // Fade out rain, fade in overlay
    ...
    overlay.style.cssText = [
      'position:fixed;inset:0;background:rgba(250,250,249,0.98);z-index:9999;cursor:default;',
      'display:flex;flex-direction:column;align-items:center;justify-content:center;',
      'font-family:"IBM Plex Mono",monospace;color:#111;padding:48px;overflow:auto;',
      'opacity:0;transition:opacity 0.4s ease;'
    ].join('');

    // Collect real visitor data
    ...
    var os = /Mac/i.test(ua) ? 'macOS' : /Win/i.test(ua) ? 'Windows' : /iPhone|iPad/i.test(ua) ? 'iOS' : /Android/i.test(ua) ? 'Android' : 'Unknown';
    var timeOnPage = Math.round((Date.now() - window.gtmDiagnosisStart) / 1000) || Math.floor(Math.random() * 120 + 30);
    var scrollPct = Math.round((window.scrollY / (document.body.scrollHeight - window.innerHeight)) * 100) || 0;

    // Score — fun fake assessment
    ...
    html += '<div style="font-size:9px;letter-spacing:0.2em;color:rgba(232,99,74,1);margin-bottom:24px;">REVSHOPPE · GTM DIAGNOSIS · LIVE SESSION SCAN</div>';
    ...
    var stats = [
      ...
      { l: 'Browser', v: browser },
      { l: 'Viewport', v: w+'×'+h },
      { l: 'Page Read', v: scrollPct+'%' },
    ];
    stats.forEach(function(s) {
      html += '<div style="background:rgba(0,0,0,0.03);padding:16px 20px;">';
      html += '<div style="font-size:8px;letter-spacing:0.14em;color:rgba(0,0,0,0.35);text-transform:uppercase;margin-bottom:6px;">'+s.l+'</div>';
      html += '<div style="font-size:15px;color:#111;">'+s.v+'</div>';
      html += '</div>';
    });
    html += '</div>';

    // Four pillar findings
    html += '<div style="display:flex;flex-direction:column;gap:1px;margin-bottom:32px;">';
    findings.forEach(function(f) {
      html += '<div style="display:flex;align-items:center;background:rgba(0,0,0,0.03);padding:14px 20px;border-radius:4px;">';
      html += '<div style="font-size:8px;letter-spacing:0.14em;color:rgba(232,99,74,0.7);text-transform:uppercase;width:90px;flex-shrink:0;">'+f.pillar+'</div>';
      html += '<div style="flex:1;font-size:12px;color:rgba(0,0,0,0.5);">'+f.label+'</div>';
      html += '<div style="font-size:16px;color:#111;margin:0 20px;">'+f.val+'</div>';
      html += '<div style="font-size:8px;letter-spacing:0.1em;color:rgba(232,99,74,0.9);border:1px solid rgba(232,99,74,0.3);padding:3px 8px;border-radius:100px;">'+f.status+'</div>';
      html += '</div>';
    });
    html += '</div>';

    // Score
    html += '<div style="display:flex;align-items:center;justify-content:space-between;border-top:1px solid rgba(0,0,0,0.08);padding-top:24px;">';
    ...
    html += '<a href="https://revshoppe.com/contact" style="background:var(--accent,#E8634A);color:#fff;text-decoration:none;cursor:pointer;font-size:10px;letter-spacing:0.1em;text-transform:uppercase;padding:12px 24px;border-radius:6px;">Book a Real One →</a>';
    html += '</div>';

    // Close hint
    html += '<div style="text-align:center;margin-top:28px;font-size:9px;color:rgba(0,0,0,0.2);letter-spacing:0.1em;">PRESS ESC OR CLICK ANYWHERE TO CLOSE</div>';
    html += '</div>';
    ...
    scanLine.style.cssText = 'position:absolute;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(232,99,74,0.35),transparent);top:0;transition:top 3s ease;pointer-events:none;';
    overlay.appendChild(scanLine);

    requestAnimationFrame(function() {
      overlay.style.opacity = '1';
      setTimeout(function() {
        scanLine.style.transition = 'top 3s ease';
  ...
  // Green dot in readout frame triggers diagnosis
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (greenDot) {
      ...
      greenDot.addEventListener('mouseenter', function() {
        this.style.filter = 'brightness(1.3)';
      });
      greenDot.addEventListener('mouseleave', function() {
        this.style.filter = '';
      });
    }
  });
